    ...
    getNotifications: builder.query({
      ...
      async onCacheEntryAdded(
        ...
        try {
          ...
          // When data is received from the socket connection to the server, update out query result with the received message
          const listener = (event) => {
            const message = JSON.parse(event.data)
            switch (message.type) {
              case 'notifications': {
                updateCachedData((draft) => {
                  // Insert all received notifications from the websocket into the existing RTKQ array
